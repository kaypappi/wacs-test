        ...
        <form @submit.prevent="goToNext">
          <template v-if="steps===1">
            <div :style="{maxWidth:'400px',margin:'0 auto'}">
              ...
              >Moratorium Period (time of first repayment)</label>
        ...
        <template v-if="steps===2">
          <div :style="{maxWidth:'400px',margin:'0 auto'}">
            ...
            <div v-if="!equalRepayment" class="upload-schedule-div">
              <form @submit.prevent="goToNext">
                <p class="form-modal-label">
                  <img src="/assets/images/file.svg" alt />Enter Repayment Schedule
                </p>
                <div
                  class="schedule-wrapper"
                  :key="index"
                  v-for="(n,index) in (offer.unequal_repayment)"
                >
                  <div class="text-input">
                    ...
                    <input
                      placeholder="No"
                      :disabled="true"
                      v-model="offer.unequal_repayment[index].no"
                      type="text"
                    />
                  </div>
                  <div class="text-input">
                    <label
                      ...
                      for="repayment-month"
                    >{{index==0?'Months':''}}</label>
                    <input
                      :key="Math.random()*10+index"
                      placeholder="No"
                      :disabled="true"
                      :value="months[offer.unequal_repayment[index].month]"
                      type="text"
                    />
                  </div>
                  ...
                  <div class="text-input">
                    ...
                    <input
                      ...
                      placeholder="No"
                      :disabled="true"
                      :value="offer.unequal_repayment[index].year"
                      type="text"
                    />
                  </div>
  ...
  methods: {
    ...
    addMonth() {
      if (this.offer.unequal_repayment.length < this.offer.repayment_period) {
        ...
        const month =
          this.offer.unequal_repayment[unequal_repayment_length - 1].month == 12
            ? 1
            : this.offer.unequal_repayment[unequal_repayment_length - 1].month +
              1;
        const year =
          this.offer.unequal_repayment[unequal_repayment_length - 1].month == 12
            ? this.offer.unequal_repayment[unequal_repayment_length - 1].year +
              1
            : this.offer.unequal_repayment[unequal_repayment_length - 1].year;
        const no =
          parseInt(
            this.offer.unequal_repayment[unequal_repayment_length - 1].no
          ) + 1;
        this.offer.unequal_repayment = [
          ...this.offer.unequal_repayment,
          { month, year, no, amount: "" }
        ];
      }
    },
    checkProperties(obj) {
    ...
    validateEqual() {
      ...
      if (
        this.getTotalEqualRepaymentAmount() <
        this.stripString(this.offer.loan_amount)
      ) {
        this.errors.step2.equal =
          "Total repayment amount cannot be less than loan amount";
        return false;
      }
      this.errors.step2.equal = "";
      return true;
      ...
      if (this.formValues) {
        ...
        if (!valid) {
          this.errors.step2.unequal =
            "All fields are required for manual schedule entry";
        } else if (
          this.getTotalCsvRepaymentAmount() <
          this.stripString(this.offer.loan_amount)
        ) {
          valid = false;
          this.errors.step2.unequal =
        ...
        if (!valid) {
          ...
          this.getTotalUnequalRepaymentAmount() <
          this.stripString(this.offer.loan_amount)
        ) {
          valid = false;
          this.errors.step2.unequal =
    ...
    getTotalEqualRepaymentAmount() {
      return (
        this.stripString(this.offer.repayment_amount) *
        this.offer.repayment_period
      );
    },
    getTotalCsvRepaymentAmount() {
      return this.offer.csv_repayment.reduce((a, b) => ({
        amount:
          parseFloat(this.stripString(a.amount)) +
          parseFloat(this.stripString(b.amount))
      })).amount;
    },
    getTotalUnequalRepaymentAmount() {
      return this.offer.unequal_repayment.reduce((a, b) => ({
        amount:
          parseFloat(this.stripString(a.amount)) +
          parseFloat(this.stripString(b.amount))
      })).amount;
      ...
      if (this.equalRepayment) {
        ...
        if (this.offer.csv_repayment.length > 0) {
          ...
          data.plan = this.offer.unequal_repayment.map(item => {
            return {
              ...item,
              amount: this.stripString(item.amount),
              month: moment()
                .month(item.month)
                .format("MMMM")
            };
    ...
    populateUnequalRepayment() {
      ...
      for (let index = 0; index < this.offer.repayment_period; index++) {
        let month = (startMonth + index) % 12,
          year = startYear + Math.floor((startMonth + index) / 12);
        const obj = {
          no: index + 1,
          month: month,
          year,
